...
newtype Uri = Uri { unUri :: Text } deriving Show
newtype Path = Path { unPath :: Text } deriving Show
newtype RoutingKey = RoutingKey { unKey :: Text } deriving Show
...
outfile = Outfile . Path <$> strOption
            (long "outfile" <>
             short 'o' <>
             metavar "OUTPUT_FILE" <>
...
exchange = Exchange . Uri <$> strOption
            (long "dsturi" <>
...
runArgParser = customExecParser p opts
        where
            things = header "bobek - RabbitMq Swiss Army Knife" <>
